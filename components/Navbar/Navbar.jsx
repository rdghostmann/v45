    ...
    <nav className="w-full bg-gradient-to-tr from-blue-700 to-red-500 text-white sticky top-0 z-10">
      <section className="max-w-7xl mx-auto py-2 px-4 lg:p-4 flex justify-between items-center">
        ...
        <div className="text-2xl font-medium">
          <button type="button" id="hamburger-button" className="block text-2xl lg:hidden  cursor-pointer" onClick={toggleMobileMenu}>☰</button>
          ...
          <nav aria-label="main" className="hidden lg:block space-x-8 text-xl">
            <Link href="/login" className="text-base font-normal hover:opacity-90">Login</Link>
            <Link href="/register" className="border border-white px-4 py-2 rounded-3xl text-base font-normal hover:opacity-90">Register</Link>
          </nav>
      </section>
      ...
      {isMobileMenuOpen && (
        <section id="mobile-menu" className="absolute top-0 bg-black w-full text-xl flex-col justify-content">
          ...
          <nav className="flex flex-col min-h-screen item-center py-4" aria-label="mobile">
            ...
            <Link onClick={toggleMobileMenu} href="/login" className="w-full lg:text-3xl text-center py-4 hover:opacity-90 border-t">Login</Link>
            <Link onClick={toggleMobileMenu} href="/login" className="w-full lg:text-3xl text-center py-4 hover:opacity-90"><span className='border border-white py-2 px-6 rounded-3xl'>Register</span></Link>
          </nav>
        </section>
      )}
